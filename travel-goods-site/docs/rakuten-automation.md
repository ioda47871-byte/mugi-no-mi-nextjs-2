...
#### 未解決: Referer を送っても 403 が続いています（2026-08-31 時点）

固定ドメイン（`https://travel-goods-site.vercel.app/`）を楽天のアプリ登録2項目と
`RAKUTEN_API_REFERER` の両方に設定しても、`REQUEST_CONTEXT_BODY_HTTP_REFERRER_MISSING`
が返り続けています。ここまでに確認できたことを残します。

| 確認 | 結果 |
|---|---|
| ジョブに値が届いているか | 届いている（ログの「送信元(Referer)」に出る） |
| クライアントが実際に送っているか | **送っている**。ローカルのHTTPサーバーに実クライアントを向けて、`referer` と `accesskey` が届くことを実測 |
| キーの誤りか | 別物。無効なキーなら 403 `Invalid Access Key` が返る（架空の値で確認） |
| 正規化のズレか | `new URL().toString()` による末尾スラッシュ補完をやめ、設定文字列をそのまま送るよう修正済み |

残る可能性（未検証）:

- 楽天側のアプリ設定変更が反映されるまでの時間差
- 登録値と送信値の細かな不一致（末尾スラッシュの有無など）。Variable の値を
  変えるだけで試せます
- サーバーからの利用に、アプリ登録側で別の設定（利用条件・アクセス制限）が要る

**この節が「解決済み」に書き換わるまで、自動取得は動かせません。**
