...
- **本番データは 0 件です。** 実装環境から外部サイトへアクセスできず、
  メーカー公表仕様を確認できなかったため、確認していない値を書き込んでいません。
- 画面と機能の確認には、実在しない架空データ（`datasets/demo/`）を使います。
...
| `npm run test:e2e` | E2E（Playwright / desktop 1440px・mobile 360px） |
...
すべてを満たしたリンクだけを表示します。1つでも欠ければボタンを出さず、
理由を画面に表示します。ダミーURLや `#` は使いません。
