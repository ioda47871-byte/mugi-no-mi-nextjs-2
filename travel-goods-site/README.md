...
現時点は **「サイト基盤完成、実データの投入が進行中」** です。
`docs/status.md` に、**実装完了 / 内容準備完了 / 収益化設定完了 / 本番公開完了**を
分けて記録しています。要約すると:

- 実装は完了しています（全画面・絞り込み・比較表・購入リンク解決・公開判定・CLI・テスト）。
- **実商品は 23 件**（公開22件 / review 1件）、**公開記事は 7 本**です。
  計画書の目標（約30商品・10記事）にはまだ届いていません。
...
- 仕様値の 20 件分の出典は、メーカー公式ページを**自分で取得**したものです
  （`provenance: direct-fetch`）。エレコムの3商品は公式サイトが 403 を返すため、
  提供資料からの取り込み（`provided-document`）のままです。
  **外部への到達性は実行環境ごとに変わります**（`docs/status.md` の点検記録を参照）。
- **表示できる購入リンクは 14 件**です（楽天）。残る1件（クレスタS 09162）は指定色が
  再入荷待ちのため `unverified` のままで、購入ボタンを出していません。
...
| `npm run link:pending` | 確認待ちのリンクを、確認用の商品ページURLつきで一覧表示 |
