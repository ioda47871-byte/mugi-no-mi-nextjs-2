  ...
  return (
    <section className="bg-white px-8 py-24 max-[640px]:px-5 max-[640px]:py-16">
      <div className="mx-auto max-w-container">
        ...
        <RevealOnScroll>
          <div className="grid grid-cols-4 gap-x-7 gap-y-12 max-[1080px]:grid-cols-3 max-[760px]:grid-cols-2 max-[760px]:gap-x-5 max-[480px]:grid-cols-1">
            {products.map((product, i) => (
              ...
              >
                <div className="relative mb-4 overflow-hidden rounded-[2px] shadow-[0_2px_10px_rgba(43,36,29,0.05)]">
                  <PhotoFrame
                    src={product.image}
                    alt={product.name}
                    aspect="aspect-[4/5]"
                    ...
                    priority={i < 4}
                    sizes="(max-width: 480px) 90vw, (max-width: 760px) 45vw, (max-width: 1080px) 30vw, 22vw"
                  />
                  {product.isSoldOut && (
                ...
                <div className="flex items-baseline justify-between gap-3">
                  <h3 className="text-[16.5px] leading-snug transition-colors duration-300 group-hover:text-brand-deep">
                    {product.name}
                  </h3>
                  <span className="shrink-0 font-accent text-[18px] text-brand-text">
                    ¥{product.price.toLocaleString()}
                  </span>
                </div>
                <p className="mt-2 text-[13px] text-kura">{product.description}</p>
              </Link>
            ))}
          </div>
        </RevealOnScroll>
